...
# -*- coding: utf-8 -*-
"""
build_data.py — motor de dados diario do $DOG (Radar on-chain, estilo Arkham).

Roda todo dia (GitHub Action), puxa SO de dados publicos (dogdata.xyz) e escreve
3 JSONs que o site (radar.js) le:

  data/daily.json  -> os numeros do dia (cofre #1, holders, saldos, leitura honesta)
  data/feed.json   -> o feed de "noticias on-chain" (cada movimento vira um card)
  data/graph.json  -> nos + arestas do grafo (Cofre #1 no centro)
  data/state.json  -> estado entre execucoes (txids ja vistos) — VERSIONADO de proposito

Principio editorial: so afirmamos o que a chain prova. Rotulos de corretora sao da
COMUNIDADE e vem marcados como "nao confirmado". Sem acusacao nominal como fato.
Reproduzivel: `python3 scripts/build_data.py`.
"""
...
NODES = [
    {"id": "cofre", "addr": "bc1plzs2lltvv29k603w5m0aqma5e8w0n3pc77dt89l5w9hurmdfgd0swdhspn",
     "label": "Cofre #1", "kind": "whale", "community": None, "feed": True},
...
def label_for(addr):
    """Rotulo amigavel de um endereco (no vigiado) ou 'carteira nova' + curto."""
    n = BY_ADDR.get(addr)
    if n:
...
def run():
    ...
    for n in NODES:
        ...
        nodes_out.append({
            ...
            "addr": n["addr"]})
    # destinos externos (carteiras novas) que mais receberam do cofre -> so os top 5
    ext = {}
    ...
    if node["kind"] == "whale":
        if direction == "out":
            ...
            return mk("cofre_out_new", amt, ts, txid, "watch",
                      node["label"], "carteira nova", "cofre", cp, None)
        else:
